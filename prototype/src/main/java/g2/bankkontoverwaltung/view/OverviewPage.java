...
public class OverviewPage implements ActionListener{
	...
	public JFrame frame = new JFrame("Accounts overview");
		...
		if(e.getSource()==giroButton) {
			...
			for (Konto konto: konten) {
				if (konto instanceof Girokonto) {
					...
					JOptionPane.showMessageDialog(null, ("account ID: "+konto.getId()+"\n"+"IBAN: "+((Girokonto)konto).getIban()+"\n"+"balance: "+konto.getSaldo()), "Giro accounts", JOptionPane.INFORMATION_MESSAGE);
				}
			}
			...
			for (Konto konto: konten) {
				if (konto instanceof Depotkonto) {
					System.out.println(konto.getId());
					System.out.println(konto.getSaldo());
					JOptionPane.showMessageDialog(null, ("account ID: "+konto.getId()+"\n"+"balance: "+konto.getSaldo()), "Depot accounts", JOptionPane.INFORMATION_MESSAGE);
				}
			}
